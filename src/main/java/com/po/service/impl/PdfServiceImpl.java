...
public class PdfServiceImpl implements PdfService {
	...
	public String createPDF(String path) {
		...
		try {
			...
			Font font = new Font(baseFont, 5); // 字体、字号
			Font fontBold = new Font(baseFont, 5, Font.BOLD); // 字体、字号、黑体
			...
			cells21[0] = new PdfPCell(new Paragraph(
					"4.所有货物包装必须按照国家有关部门的规定进行标识，卖方保证以上价格已包含上述包装所需的费用。", font));
			...
			logger.error("生成pdf异常：" + e.getMessage());
		} finally {
			document.close(); // 关闭文档
			writer.close();// 关闭书写器
		}
